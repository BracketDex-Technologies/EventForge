...
export default function CampaignRow({ eventId, campaign }: CampaignRowProps) {
  ...
  const getChannelBadge = (ch: string) => {
    switch (ch) {
      case 'whatsapp':
        return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800 border border-emerald-250">WhatsApp</span>;
      case 'push':
        return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-purple-100 text-purple-800 border border-purple-250">Push</span>;
      default:
        return <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-800 border border-slate-250">Email</span>;
    }
  };
  ...
  return (
    <tr className="hover:bg-slate-50 transition-colors">
      <td className="py-4 px-6 font-medium text-slate-900">
        ...
        <div className="text-xs text-slate-500 font-normal mt-0.5">{subjectText || 'No subject line'}</div>
      </td>
